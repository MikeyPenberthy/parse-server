...
// Parse.Cloud.define("testIncrement", function(request, response) {
// 	Parse.Cloud.useMasterKey();
// 	var user = new Parse.User();
// 	user.id = "0uRzPMKGKx";
// 	user.increment("achievements.appRunTime", 1);
// 	user.save().then(function(saved) {
// 		response.success("Succesfully incremented user");
// 	}, function(err) {
...
Parse.Cloud.beforeSave(Parse.User, function(request, response) {
	...
	if (user.get("verified") && user.dirty("username")) {
		return response.error("Cannot update username for a verified user");
	}
	response.success();
});
...

{
	...
	members : [
		{
			"id" : "59gbn4w948, // The member ID
			"userId" : 4t0gestjeg54",
			"status" : "active",
			"groupId" : "350g8slff5jt"	
			"phone" : "2453950384",
			"designatedDriver" : false,
			"notifications" : ["message", "member_drink", "member_joined", "member_passed_limit"], // ONLY for current user
...
Parse.Cloud.beforeSave("GroupMember", function(request, response) {
	...
	if (!member.isNew()) {
		...
		// make sure it matches the original phone invite.
		// The user can change though in the case of a non-user invite
		// if (user && user.get("username") != member.get("phone")) {
		// 	return response.error("The user's phone number must match the member phone");
		// }
	}
...
/*
** Send Group Invites Helper
** ----------------------------------------
** Create GroupMembers for a provided list of invites
** groupIsNew to bypass the unique validation check
**/
